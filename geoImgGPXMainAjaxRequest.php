...
function geoimggpx_ajax_initAdminPostPage_request(){
	if (
		...
		echo "<span>".__("You do not have enough rights to use this plugin. You must have at least the administrator, editor or author role.", "gm")."</span>";
		wp_die();
	}
	// Verify that the nonce is valid.
	if ( ! wp_verify_nonce( $_POST['nonce'], 'geoImgGPX_meta_box' ) ) {
		echo "<span>".__("Error in nonce", "gm")."</span>";
		wp_die();
	}
