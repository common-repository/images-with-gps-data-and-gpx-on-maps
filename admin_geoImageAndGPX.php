...
function geoImgGPXAdminPage(){
	...
	if( $exif_read_data_func == false and  $exiftool_func == false){
		...
		echo "<span class='gm_info'>".__("Ihr Server unterstütz EXIF-Applikationen zum auslesen von GeoDaten:", "gm")."</span>";
		echo "<br>";
		if( $exif_read_data_func == false){
			echo "<span class='gm_warning'>".__("- exif_read_data() funktioniert nicht.", "gm")."</span>";
		}else{
			echo "<span class='gm_info'>".__("- exif_read_data()  funktioniert und extrahiert GPS Daten aus den Bildern.", "gm")."</span>";
		}
		echo "<br>";
		...
		if($geoImgGPX_ExifTool_path !== NULL && strlen($geoImgGPX_ExifTool_path) > 0){
			if( $exiftool_func == false){
				...
				echo "<span class='gm_info'>".__("- ExifTool funktioniert und extrahiert GPS Daten aus den Bildern.", "gm")."</span>";
			}
		}
		echo "<hr>";
	}
